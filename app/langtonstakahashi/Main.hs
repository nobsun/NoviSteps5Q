...
type Solver = (I,I,I) -> [[Char]]

solve :: Solver
solve = \ case
    (h,w,n) -> iter n b0 (p0,d0) where
        b0 = M.fromList (map (,'.') (range ((0,0),(pred h,pred w))))
        p0 = (0,0)
        d0 = N
        iter 0 b _ = splitEvery w $ M.elems b
        iter k b (p,d) = iter (pred k) (M.update inv p b) (next b (p,d))
        inv = \ case
            '.' -> Just '#'
            _   -> Just '.'
        next b (p,d) = case M.lookup p b of
            Just '.' -> case d of
                N -> ((second $ (`mod` w) . succ) p, E)
                E -> ((first  $ (`mod` h) . succ) p, S)
                S -> ((second $ (`mod` w) . pred) p, W)
                W -> ((first  $ (`mod` h) . pred) p, N)
            Just '#' -> case d of
                S -> ((second $ (`mod` w) . succ) p, E)
                W -> ((first  $ (`mod` h) . succ) p, S)
                N -> ((second $ (`mod` w) . pred) p, W)
                E -> ((first  $ (`mod` h) . pred) p, N)
            _ -> invalid
