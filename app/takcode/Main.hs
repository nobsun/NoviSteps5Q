...
trim :: [String] -> (Int,Int) -> [String]
trim b (i,j) = map (take 9 . drop j) $ take 9 $ drop i b
...
solve :: Solver
solve = \ case
    (n,m,bss) -> case range ((0,0),(n-9,m-9)) of
        ijs -> map conv $ filter check ijs
        where
            check (i,j) = phi template (trim bss (i,j))
            phi = (and .) . zipWith ((and .) . zipWith match)
...
conv :: (Int,Int) -> [Int]
conv (x,y) = [succ x,succ y]

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    ls:bss -> case map read (words ls) of
        [n,m]  -> f (n,m,bss)
