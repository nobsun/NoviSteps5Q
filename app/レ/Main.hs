...
solve :: Solver
solve = \ case
    (n,as) -> iter False [] [] [1..n] as
    where
        iter _   s t []     _      = concat (reverse (t:s))
        iter flg s t (i:is) jjs@(j:js) = if
            | i == j    -> iter True  s         (i:t) is js 
            | flg       -> iter False ((i:t):s) []    is jjs
            | otherwise -> iter False ([i]:t:s) []    is jjs
        iter False s t iis []
            = iter False (iis:t:s) []    [] []
        iter True  s t (i:is) []
            = iter False ((i:t):s) [] is []
