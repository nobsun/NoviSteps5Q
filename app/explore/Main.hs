...
solve :: Solver
solve = \ case
    (t,as,bs) -> iter 1 t bs as
    where
        iter _ a _  []     = bool "No" "Yes" (0 < a)
        iter i a xs (y:ys) = if a <= 0 then "No"
            else case xs of
                [] -> iter (succ i) (a - y) [] ys
                (j,b):xs'
                    | i == j    -> iter (succ i) (a + b - y) xs' ys
                    | otherwise -> iter (succ i) (a - y)     xs  ys

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [_,_,t]:as:bss -> case f (t,as,toTuple <$> bss) of
        rr -> [[rr]]
