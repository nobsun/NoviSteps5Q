...
solve :: Solver
solve bss = case denvelop bss of
    ((ns,ss),(ws,es),kss) -> case 4 * len of
        rlen -> case cycle (ns ++ es ++ concatMap reverse [ss, ws]) of
            ring -> case drop (rlen - 1) ring of
                newRing -> case decomp (rlen `div` 4) newRing of
                    (ubs,lrs) -> envelop kss (ubs,lrs)
        where
            len = pred (length ns)

decomp :: Int -> [a] -> (([a],[a]),([a],[a]))
decomp n ring = case splitAt (succ n) ring of
    (ns,rs) -> case splitAt (pred n) rs of
        (es,rs') -> case first reverse $ splitAt (succ n) rs' of
            (ss,rs'') -> case first reverse $ splitAt (pred n) rs'' of
                (ws,_)    -> ((ns,ss),(ws,es))

envelop :: [[a]] -> (([a],[a]),([a],[a])) -> [[a]]
envelop kss ((ns,ss),(ws,es))
    = sandwich (ns,ss) (transpose (sandwich (ws,es) (transpose kss)))

sandwich :: (a,a) -> [a] -> [a]
sandwich (x,y) zs = x:(zs ++ [y])

peri :: [a] -> (a,[a],a)
peri xs = case length xs of
    len | len < 2    -> error "too short"
        | otherwise  -> case splitAt 1 xs of
            (x:_, rs)    -> case splitAt (len - 2) rs of
                (ys,z:_)     -> (x,ys,z)
                _            -> impossible
            _            -> impossible

denvelop :: [[a]] -> (([a],[a]),([a],[a]),[[a]])
denvelop ms = case peri ms of
    peris -> case peris of
        (ns,css,ss) -> case unzip3 $ map peri css of
            (ws,kss,es) -> ((ns,ss),(ws,es),kss)
