...
solve :: Solver
solve = \ case
    (n,bss) -> S.size $ us `S.difference` S.unions (mkpairs <$> bss)
        where
            us = S.fromList [(i,j) | i <- [1 .. n], j <- [succ i .. n] ]

mkpairs :: [Int] -> S.Set (Int,Int)
mkpairs s = S.fromList (zipWith mkpair s (tail s))

mkpair :: Int -> Int -> (Int, Int)
mkpair x y = (min x y, max x y)

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n,_]:bss -> case f (n,bss) of
        r -> [[r]]
