...
solve :: Solver
solve = \ case
    (n,m,bss,css) -> case range ((0,0),(n-m,n-m)) of
        offs -> bool "No" "Yes" $ any p offs where
            p (i,j) = (==) css $ map (take m . drop j) $ take m $ drop i bss

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    nm:abss -> case read <$> nm of
        [n,m]   -> case splitAt n (concat abss) of
            (bss,css) -> case f (n,m,bss,css) of
                r -> [[r]]
