...
solve :: Solver
solve = \ case
    (h,w,bss,css) -> bool "No" "Yes"
               $ css `elem` map (shift (h,w) bss) (range ((0,0),(pred h, pred w)))

shift :: (Int, Int) -> [[a]] -> (Int, Int) -> [[a]]
shift (h,w) bss (i,j) = take h $ drop i $ take w . drop j <$> cycle (cycle <$> bss)


wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    cs:sss -> case read <$> words cs of
        [h,w] -> case splitAt h sss of
            (bss,css) -> case f (h,w,bss,css) of
                r -> [[r]]
