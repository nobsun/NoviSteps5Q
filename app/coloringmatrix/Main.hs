...
solve :: Solver
solve = \ case
    (as,bs) -> maybe "No" (const "Yes") (find (check bs) $ take 4 (iterate rot as))

check :: [[Int]] -> [[Int]] -> Bool
check xs ys = and (zipWith eql xs ys)

eql :: [Int] -> [Int] -> Bool
eql xs ys = and (zipWith eqi xs ys)
...
wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n]:sss -> case splitAt n sss of
        (as,bs) -> case f (as,bs) of
            r -> [[r]]
