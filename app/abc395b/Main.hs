...
solve = \ case
    1             -> ["#"]
    n | odd n     -> squares n
      | otherwise -> case squares (succ n) of
        ss -> case splitAt m  ss of
            (ts,_:us) -> case splitAt m $ transpose (ts ++ us) of
                (ts',_:us') -> transpose (ts' ++ us')
                _           -> invalid
            _ -> invalid
        where 
            m = n `div` 2
...
squares :: Int -> [[Char]]
squares n = splitEvery n 
          $ elems 
          $ accumArray (const id) ' ' ((1,1),(n,n)) 
          $ concatMap phi [1 .. n] where
    phi = \ case
        i | i > j     -> []
          | otherwise -> zip (range ((i,i),(j,j))) (repeat (bool '.' '#' (odd i)))
            where
                j = succ n - i
