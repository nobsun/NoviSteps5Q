...
solve :: Solver
solve = \ case
    (h,w,bss) -> case listArray ((1,1),(h,w)) (concat bss) of
        ba -> case horizs =<< [ (i,j) | i <- [1 .. h], j <- [1 .. succ w - 5]] of
            hss -> case verts =<< [ (i,j) | i <- [1 .. succ h - 5], j <- [1 .. w]] of
                vss -> case rldiags =<< [ (i,j) | i <- [1 .. succ h - 5], j <- [5 .. w]] of
                    rlss -> case lrdiags =<< [ (i,j) | i <- [1 .. succ h - 5], j <- [1 .. succ w - 5]] of
                        lrss -> case concat [hss,vss,rlss,lrss] of
                            ixss -> maybe invalid (map fromTuple) (find (snukep ba) ixss)

fromTuple :: (a,a) -> [a]
fromTuple = \ case
    (x,y) -> [x,y]

horizs, verts, rldiags, lrdiags :: (Int,Int) -> [[(Int,Int)]]
horizs sp = (:) <*> (:[]) . reverse $ take 5 (iterate (second succ) sp)
verts  sp = (:) <*> (:[]) . reverse $ take 5 (iterate (first  succ) sp)
rldiags sp = (:) <*> (:[]) . reverse $ take 5 (iterate (succ *** pred) sp)
lrdiags sp = (:) <*> (:[]) . reverse $ take 5 (iterate (succ *** succ) sp)

snukep :: Array (Int,Int) Char -> [(Int,Int)] -> Bool
snukep ba is = "snuke" == map (ba !) is

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    s:bss -> case read <$> words s of
        [h,w] -> f (h,w,bss)
