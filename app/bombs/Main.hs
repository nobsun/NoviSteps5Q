...
solve :: Solver
solve = \ case
    (r,c,css) -> case zip (range ((1,1),(r,c))) (concat css) of
        tss -> case foldl' phi (S.empty, S.empty) tss of
            (ws,bs) -> case S.difference ws bs of
                ws' -> splitEvery c $ map (conv ws') tss
        where
            phi (ws,bs) (p,e) = case e of
                '.' -> (ws,bs)
                '#' -> (S.insert p ws, bs)
                d | isDigit d 
                    -> (ws,foldl (flip S.insert) bs (expand (r,c) p (digitToInt d)))
                _ -> invalid
            conv tb (p,_) = bool '.' '#' (S.member p tb)

expand :: (Int,Int) -> (Int,Int) -> Int -> [(Int,Int)]
expand (h,w) (r,c) d = [ (i,j) | i <- [r-d .. r+d], 0 < i, i <= h
                               , j <- [c-d .. c+d], 0 < j, j <= w
                               , abs (r - i) + abs (c - j) <= d
                       ]
