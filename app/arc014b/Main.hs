...
solve :: Solver
solve = \ case
    ws -> iter S.empty (head (head ws)) True ws where
        iter ss prev turn = \ case
            [] -> "DRAW"
            x:xs -> case x of
                c:_ | c /= prev || S.member x ss -> bool "WIN" "LOSE" turn
                    | otherwise -> iter (S.insert x ss) (last x) (not turn) xs
                _ -> invalid

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    _:wss -> case f (concat wss) of
        r -> [[r]]
