...
solve :: Solver
solve = \ case
    (t,cs,rs) -> case zipWith3 (\ i c r -> (i,(c,r))) [1..] cs rs of
        tbl -> case filter ((t ==) . fst . snd) tbl of
            [] -> fst $ maximumBy (comparing (snd . snd)) 
                $ filter ((fst (snd (head tbl)) ==) . fst . snd) tbl 
            ws -> fst $ maximumBy (comparing (snd . snd)) ws

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [_,t]:cs:rs:_ -> case f (t,cs,rs) of
        rr -> [[rr]]
