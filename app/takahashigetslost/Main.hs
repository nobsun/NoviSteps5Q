...
solve :: Solver
solve = \ case
    (h,w,t,ss) -> length (mapMaybe cp sps)
        where
            sps = [ p | (p,b) <- assocs ba, b ]
            cp sp = foldM mv sp t
            ba = listArray ((1,1),(h,w)) $ (conv <$>) =<< ss
            conv = \ case
                '.' -> True
                _   -> False
            mv p d = case d of
                'L' -> bool Nothing (Just qL) (ba ! qL)
                'R' -> bool Nothing (Just qR) (ba ! qR)
                'U' -> bool Nothing (Just qU) (ba ! qU)
                _   -> bool Nothing (Just qD) (ba ! qD)
                where
                    qL = second pred p
                    qR = second succ p
                    qU = first pred p
...
wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [h,w,_]:[t]:ss -> case f (read h,read w,t,concat ss) of
        r -> [[r]]
