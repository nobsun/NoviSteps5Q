...
solve :: Solver
solve = \ case
    [s,t]:sts -> bool "No" "Yes" $ explore (S.singleton s) sts || explore (S.singleton t) sts
    _ -> invalid

explore :: S.Set String -> [[String]] -> Bool
explore s = \ case
    []       -> True
    [t,u]:us -> S.notMember t s && S.notMember u s
             && explore (S.insert u (S.insert t s)) us
    _ -> invalid
