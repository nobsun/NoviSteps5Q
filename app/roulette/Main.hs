...
type Solver = (I,[(I,[I])]) -> (O,[O])

solve :: Solver
solve = \ case
    (x,cass) -> case mapMaybe phi (zip [1 ..] cass) of
        rs -> case list [] const $ groupBy ((==) `on` snd) $ sortBy (comparing snd) rs of
            ss -> (length ss, fst <$> ss)
        where
            phi (i, (c,as)) = bool Nothing (Just (i,c)) (x `elem` as)

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n]:rss -> case splitAt n $ splitEvery 2 rss of
        (xss,[[[x]]]) -> case map phi xss of
            cass -> case f (x,cass) of
                (k,bs) -> [[k],bs]
        _           -> invalid
